"""页面 4：数据分析 —— 操作区 + Tabs 结果区（汇总/排名图表/趋势/散点）。"""
    ...
    if st.session_state.analysis is None or st.session_state.analysis_key != cache_key:
        with st.spinner("正在计算统计…"):
            try:
                ...
                handle_exception(
                    exc, logger, "统计计算", message="统计计算失败，请调整选择后重试。"
                )
                st.stop()
...
with tab_summary:
    if dimension is not None:
        st.markdown(f"**按「{dimension}」分组汇总**")
    ...
    st.caption(
        "统计口径与 Excel 一致：求和/平均忽略空值；若指标列尚未归一化，结果可能不含文本金额。"
    )

# Tab 2：排名与图表
with tab_rank:
    if dimension is None or agg != "sum":
        st.caption("请在上方选择**维度**且聚合方式为**求和**，以查看排名与图表。")
    else:
...
with tab_trend:
    if not date_candidates:
        st.caption("未识别到日期列，无法生成时间趋势。")
    elif metric not in numeric_candidates:
